/*
 * AuthModalV2 — a conta de cliente por CPF, com senha + código no WhatsApp ou e-mail.
 * Plano de 09/09/2026 (_docs/plano-login-cpf-whatsapp.md). Só aparece nos eventos
 * com `auth_flow = 'v2'`; o AuthModal antigo continua nos demais.
 *
...
export function AuthModalV2({ isOpen, onClose, onAuthenticated }: AuthModalV2Props) {
  ...
  useEffect(() => {
    if (user && isOpen) onAuthenticated();
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      ...
      >
        ...
        <div className={cn('flex flex-col', isMobile ? 'h-screen' : 'max-h-[85vh]')}>
          {/* Cabeçalho */}
          <div className={cn('border-b border-border/50', isMobile ? 'sticky top-0 z-10 bg-card/95 backdrop-blur-xl px-4 pt-4 pb-4' : 'px-6 pt-6 pb-4')}>
