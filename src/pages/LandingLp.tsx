...
/* Apresentação comercial (PDF versionado em public/ — troca de arquivo = publish). */
const APRESENTACAO_PDF = '/apresentacao/FestPag-Apresentacao.pdf';
...
const LP3_CSS = `.lp3 *,.lp3 *::before,.lp3 *::after{box-sizing:border-box;margin:0;padding:0}.lp3{
 ...
 padding:13px 22px;transition:transform .25s var(--ease),box-shadow .25s var(--ease),background .25s;position:relative;overflow:hidden}.lp3 .btn-p{background:var(--grad);color:#fff;box-shadow:0 10px 30px -12px rgba(95,110,249,.9)}.lp3 .btn-p:hover{transform:translateY(-2px);box-shadow:0 16px 40px -12px rgba(184,106,217,.95)}.lp3 .btn-s{background:rgba(242,239,249,.06);color:var(--nevoa);border:1px solid var(--line)}.lp3 .btn-s:hover{background:rgba(242,239,249,.12);transform:translateY(-2px)}.lp3 .btn-lg{font-size:17px;padding:17px 30px}.lp3 .dl{position:fixed;left:clamp(14px,2.4vw,34px);bottom:clamp(14px,2.2vw,28px);z-index:60;display:inline-flex;align-items:center;gap:9px;font-family:var(--head);font-weight:600;font-size:13.5px;line-height:1;color:var(--nevoa);text-decoration:none;white-space:nowrap;background:rgba(19,16,33,.74);border:1px solid var(--line);border-radius:999px;padding:11px 14px 11px 13px;backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);box-shadow:0 14px 34px -16px rgba(0,0,0,.9);transition:transform .25s var(--ease),background .25s,border-color .25s}.lp3 .dl:hover{background:rgba(242,239,249,.12);border-color:rgba(242,239,249,.24);transform:translateY(-2px)}.lp3 .dl svg{width:16px;height:16px;flex:none;color:var(--azulc)}.lp3 .dl-tag{font-family:var(--body);font-weight:600;font-size:10.5px;letter-spacing:.08em;color:var(--mute);border:1px solid var(--line);border-radius:999px;padding:3px 7px;margin-left:2px}.lp3 .btn:focus-visible,.lp3 a:focus-visible,.lp3 input:focus-visible,.lp3 select:focus-visible{outline:2px solid var(--azulc);outline-offset:3px}.lp3 .rail{position:fixed;right:clamp(14px,2.4vw,34px);top:50%;transform:translateY(-50%);z-index:60;display:flex;flex-direction:column;gap:16px;align-items:flex-end}.lp3 .rdot{display:flex;align-items:center;gap:10px;background:none;border:0;cursor:pointer;padding:2px}.lp3 .rdot span{font-size:11.5px;letter-spacing:.1em;text-transform:uppercase;color:var(--mute);opacity:0;transform:translateX(6px);
...
.lp3 .stage-ring{display:none}.lp3 .mod-sec{align-items:flex-end;padding-bottom:84px}.lp3 .mod-sec .mod-wrap{padding-top:52svh}.lp3 .rail span{display:none}.lp3 .wrap,.lp3 .mod-wrap{width:90vw}.lp3 .sec{padding:92px 0 64px}.lp3 .mod{max-width:none}}@media (max-width:680px){.lp3 .dl{font-size:13px;padding:10px 12px 10px 12px}.lp3 .endfoot{left:auto;right:16px;max-width:calc(100vw - 230px);text-align:right;line-height:1.35}.lp3 .nums{grid-template-columns:1fr 1fr;gap:12px}.lp3 .ncard{padding:22px 18px}.lp3 .card{padding:24px}
    ...
    <>
      ...
      <div className="lp3">
        ...
        <a
          ...
          download="FestPag-Apresentacao.pdf"
          aria-label="Baixar a apresentação comercial da FestPag em PDF"
        >
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
            <path d="M12 3v12" /><path d="m7 10 5 5 5-5" /><path d="M4 21h16" />
          </svg>
          Baixar apresentação
          <span className="dl-tag">PDF</span>
        </a>
